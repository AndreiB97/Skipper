    ...
    onBodyChange(() => {
        if (skipIntroEnabled) {
            ...
            for (let button of document.body.getElementsByTagName('button')) {
                ...
                if (buttonClass !== null && buttonClass.includes('skipelement') && button.textContent === 'Skip Intro') {
...
function skipRecap() {
    onBodyChange(() => {
        if (skipRecapEnabled) {
            ...
            for (let button of document.body.getElementsByTagName('button')) {
                const buttonClass = button.getAttribute('class');
                if (buttonClass !== null && buttonClass.includes('skipelement') && button.textContent === 'Skip Recap') {
...
async function skipper() {
    browser.storage.onChanged.addListener((change) => {
        ...
        if (change.hasOwnProperty(skipPrimeRecap)) {
            skipRecapEnabled = change[skipPrimeRecap].newValue;
        }
    ...
    skipRecapEnabled = await getConfigFromStorage(skipPrimeRecap);
