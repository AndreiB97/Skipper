...
function skipIntro() {
    onBodyChange(() => {
        if (skipIntroEnabled) {
            const elem = document.querySelector('button[data-uia="player-skip-intro"]');
...
function skipRecap() {
    onBodyChange(() => {
        if (skipRecapEnabled) {
            const elem = document.querySelector('button[data-uia="player-skip-recap"]');
...
async function skipper() {
    ...
    // for some reason this gets the script stuck if done outside a function
